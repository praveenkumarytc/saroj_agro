...
class _CategoryProductScreenNewState extends State<CategoryProductScreenNew> {
  void _loadData(BuildContext context) async {
    if (Provider.of<CategoryProvider>(context, listen: false).categorieselectedIndex == -1) {
      ...
      Provider.of<CategoryProvider>(context, listen: false).getSubCategoryList(context, widget.categoryId, Provider.of<LocalizationProvider>(context, listen: false).locale.languageCode);
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.subCategoryName != null && widget.subCategoryName != 'null') {
      ...
      appBarText = Provider.of<CategoryProvider>(context).categoryModel != null ? Provider.of<CategoryProvider>(context).categoryModel!.name! : 'name';
    }
    Provider.of<ProductProvider>(context, listen: false).initializeAllSortBy(context);
    return Scaffold(
      appBar: (ResponsiveHelper.isDesktop(context)
          ? const PreferredSize(preferredSize: Size.fromHeight(prefferedSized), child: WebAppBar())
          : CustomAppBar(
              title: appBarText,
              isCenter: false,
              isElevation: true,
                                      ...
                                      Padding(
                                        ...
                                        child: InkWell(
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(color: categoryProvider.categorieselectedIndex == -1 ? Theme.of(context).primaryColor : ColorResources.getGreyColor(context), borderRadius: BorderRadius.circular(7)),
                                            child: Text(
                                          ...
                                          itemBuilder: (BuildContext context, int index) {
                                            return InkWell(
                                              ...
                                              child: Container(
                                                ...
                                                alignment: Alignment.center,
                                                margin: const EdgeInsets.only(right: Dimensions.paddingSizeSmall),
                                                decoration: BoxDecoration(color: categoryProvider.categorieselectedIndex == index ? Theme.of(context).primaryColor : ColorResources.getGreyColor(context), borderRadius: BorderRadius.circular(7)),
                                                child: Text(
                            ...
                            child: productProvider.hasData!
                                ? Padding(
                                    ...
                                    child: ProductShimmer(
                                      isEnabled: Provider.of<ProductProvider>(context).categoryProductList.isEmpty,
                                    ),
                                  )
  ...
  Widget _cartTile(BuildContext context) {
    ...
    return Consumer<CartProvider>(builder: (context, cartProvider, _) {
      ...
      (Provider.of<OrderProvider>(context).orderType == 'delivery' && !kmWiseCharge) ? deliveryCharge = Provider.of<SplashProvider>(context, listen: false).configModel!.deliveryCharge : deliveryCharge = 0;
      double itemPrice = 0;
      double discount = 0;
      double tax = 0;
                    ...
                    child: Column(
                      children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          ...
                          Text('${cartProvider.cartList.length} ${getTranslated('items', context)}', style: poppinsMedium.copyWith(color: Theme.of(context).cardColor)),
                        ]),
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
