...
class RouteHelper {
  ...
  static String getCheckoutRoute(double amount, double? discount, String? type, String code, String freeDelivery, double deliveryCharge) => '$checkout?amount=${base64Encode(utf8.encode('$amount'))}&discount=${base64Encode(utf8.encode('$discount'))}&type=$type&code=${base64Encode(utf8.encode(code))}&c-type=${base64Encode(utf8.encode(freeDelivery))}&del_char=${base64Encode(utf8.encode('$deliveryCharge'))}';
  ...
  static final Handler _createAccountHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, dynamic> params) => _routeHandler(
  ...
  static final Handler _walletHandler = Handler(
    handlerFunc: (context, Map<String, dynamic> params) => _routeHandler(child: WalletScreen(token: params['token'][0], status: params['flag'][0])),
  );
  ...
  static Widget _routeHandler({required Widget child}) {
    if (Provider.of<SplashProvider>(Get.context!, listen: false).configModel?.maintenanceMode != false) {
