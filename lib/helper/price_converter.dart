...
class PriceConverter {
  static String convertPrice(BuildContext context, double? price, {double? discount, String? discountType}) {
    ...
    return !isLeft ? '${price!.toStringAsFixed(config.decimalPointSettings!).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}' ' ${config.currencySymbol}' : '${config.currencySymbol} ' '${price!.toStringAsFixed(config.decimalPointSettings!).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }

  static Container converPriceIcon(BuildContext context, double? price, {double? discount, String? discountType, TextStyle? style, double? iconSize}) {
    if (discount != null && discountType != null) {
      if (discountType == 'amount') {
        price = price! - discount;
      } else if (discountType == 'percent') {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Text(
          price!.toStringAsFixed(config.decimalPointSettings!),
          style: style,
        )
      ]),
    );
